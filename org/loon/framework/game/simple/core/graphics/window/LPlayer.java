...
public class LPlayer extends LContainer implements IMpeg4 {
	...
	public void draw(Graphics g, int x, int y, int width, int height) {
		if (player_state == ERROR_STATE) {
			...
			if (movieScreen != null) {
				if (java2_platform) {
					if (smooth_video) {
						g.drawImage(movieScreen, x, y, width, height, null);
					} else {
						g.drawImage(movieScreen, x, y, null);
					}
				} else {
					g.drawImage(movieScreen, x, y, null);
				}
	...
	public void nextFrame(VideoFrame videoFrame) throws InterruptedIOException {
		...
		mis = new MemoryImageSource(memory_image_width, memory_image_height,
				pixels, 0, pixel_frame_width);
		mis.setAnimated(true);
		movieScreen = Toolkit.getDefaultToolkit().createImage(mis);
	...
	private void drawDownload(Graphics2D g, int x, int y) {
		if (states == null) {
			return;
		}
