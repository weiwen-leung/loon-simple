...
public class EmulatorButton {
	...
	private GLColor color = new GLColor(GLColor.gray.r, GLColor.gray.g, GLColor.gray.b,
			0.5f);
	...
	public void draw(GLEx g) {
		if (!disabled) {
			if (click && onClick) {
				if (bitmap1 != null) {
					g.drawTexture(bitmap1, bounds.x, bounds.y);
				} else {
					g.drawTexture(bitmap, bounds.x, bounds.y, color);
				}
			} else {
				g.drawTexture(bitmap, bounds.x, bounds.y);
			}
		}
	}
