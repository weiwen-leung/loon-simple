...
import org.loon.framework.android.game.core.geom.Polygon2f;
...
import org.loon.framework.android.game.utils.ColorUtils;
...
public final class GLEx implements LTrans {
	...
	private FloatBuffer texCoordBuf = BufferUtils
			.createFloatBuffer(DEFAULT_MAX_VERTICES * 2);

	private FloatBuffer vertexBuf = BufferUtils
	 ...
	 */
	public final void glColorPointer(int size, boolean b, int stride,
			ByteBuffer colorAsByteBuffer) {
		if (isClose) {
			return;
		}
		gl10.glColorPointer(size, GL10.GL_UNSIGNED_BYTE, stride,
				colorAsByteBuffer);
	}
	...
	public final void glEnd() {
		...
		if (mode == GL.GL_QUADS) {
			for (int i = 0; i < count; i += 4) {
				gl.glDrawArrays(GL11.GL_TRIANGLE_FAN, i, 4);
			}
		} else {
	...
	public GLColor getPixel(int x, int y) {
		...
		gl10.glReadPixels(x, viewPort.height - y, 1, 1, GL10.GL_RGBA,
				GL10.GL_UNSIGNED_BYTE, readBuffer);
		return new GLColor(c(readBuffer.get(0)), c(readBuffer.get(1)),
				c(readBuffer.get(2)), c(readBuffer.get(3)));
	}
	 ...
	 * @return
	 */
	public LImage getImage(int x, int y, int w, int h) {
		if (isClose) {
			return null;
		}
		LImage img = LImage.createImage(w, h, true);
		IntBuffer newBuffer = BufferUtils.createIntBuffer(w * h);
		gl10.glReadPixels(x, viewPort.height - y, w, -h, GL10.GL_RGBA,
				GL10.GL_UNSIGNED_BYTE, newBuffer);
		int[] pixels = BufferUtils.getIntArray(newBuffer);
	...
	public void getArea(int x, int y, int width, int height, ByteBuffer target) {
		...
		gl10.glReadPixels(x, y, width, height, GL10.GL_RGBA,
	...
	public void drawTriangle(GLColor color, float x, float y, float r) {
		...
		float x1 = x;
		float y1 = y - r;
		float x2 = x - (int) (r * Math.cos(Math.PI / 6));
		float y2 = y + (int) (r * Math.sin(Math.PI / 6));
		float x3 = x + (int) (r * Math.cos(Math.PI / 6));
		float y3 = y + (int) (r * Math.sin(Math.PI / 6));
	...
	public void drawRTriangle(GLColor color, float x, float y, float r) {
		...
		float x1 = x;
		float y1 = y + r;
		float x2 = x - (int) (r * Math.cos(Math.PI / 6.0));
		float y2 = y - (int) (r * Math.sin(Math.PI / 6.0));
		float x3 = x + (int) (r * Math.cos(Math.PI / 6.0));
		float y3 = y - (int) (r * Math.sin(Math.PI / 6.0));
		float[] xpos = new float[3];
		xpos[0] = x1;
		xpos[1] = x2;
		xpos[2] = x3;
	 ...
	 * @param y2
	 */
	private void $drawLine1(float x1, float y1, float x2, float y2) {
		if (x1 > x2) {
			x1++;
		} else {
			x2++;
	 ...
	 * @param y
	 */
	public void draw(final Polygon2f p, final float x, final float y) {
		if (isClose) {
			return;
		}
		gl10.glPushMatrix();
	 ...
	 * @param rotation
	 */
	public void draw(final Polygon2f p, final float rotation) {
		if (isClose) {
			return;
		}
		gl10.glPushMatrix();
	 ...
	 * @param y
	 */
	public void fill(final Polygon2f p, final float x, final float y) {
		if (isClose) {
			return;
		}
		gl10.glPushMatrix();
	 ...
	 * @param rotation
	 */
	public void fill(final Polygon2f p, final float rotation) {
		if (isClose) {
			return;
		}
		gl10.glPushMatrix();
	 ...
	 * @param p
	 */
	public void fillPolygon(Polygon2f p) {
		fillPolygon(p.xpoints, p.ypoints, p.npoints);
	}
	...
	public void fillPolygon(float xPoints[], float yPoints[], int nPoints) {
		...
		try {
			...
			case 0:
				...
				break;
			case 1:
				$fillPolygon1(xPoints, yPoints, nPoints);
				break;
			}
		} catch (Exception e) {
			switch (drawingType) {
	 ...
	 * @param p
	 */
	public void drawPolygon(Polygon2f p) {
		drawPolygon(p.xpoints, p.ypoints, p.npoints);
	}
	...
	public void drawPolygon(float[] xPoints, float[] yPoints, int nPoints) {
		...
		try {
			...
			case 0:
				...
				break;
			case 1:
				$drawPolygon1(xPoints, yPoints, nPoints);
				break;
			}
		} catch (Exception e) {
			switch (drawingType) {
	...
	public final void setViewPort(RectBox port) {
		setViewPort(port.x, port.y, port.width, port.height);
	}
	...
	private final void drawTexture(LTexture texture, float x, float y,
		...
		glTex2DEnable();
		{
			...
			{
				if (x != 0 || y != 0 || width != texture.width
				...
				glTex2DARRAYEnable();
				{
					...
					if (GLEx.vboOn) {
						...
						GLUtils.vertex2Pointer(gl10, texture.data);
						if (srcX != 0 || srcY != 0 || srcWidth != texture.width
								|| srcHeight != texture.height) {
							putRectangle(texture, srcX, srcY, srcWidth,
									srcHeight);
			...
			}
			{
				...
				glTex2DARRAYEnable();
				{
					...
					if (vboOn) {
						...
						GLUtils.vertex2Pointer(gl10, english.vertData);
						for (int i = 0; i < size; i++) {
							int tranX = 0;
							if (chars[i] == '\n') {
	...
	public final void draw(Path path) {
		path.draw(this);
	}

	public final void glTexEnvfModulateColor(float r, float g, float b, float a) {
		glTexEnvfModulateColor(r, g, b, a, true);
	}

	public final void glTexEnvfModulateColor(float r, float g, float b,
			float a, boolean update) {
		try {
			gl10.glTexEnvf(GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE,
					GL11.GL_MODULATE);
			if (update) {
				setColor(r, g, b, a);
			}
	...
	public final void glTexEnvfReplaceColor(float r, float g, float b, float a) {
	...
	public final void glUpdateModulate(float r, float g, float b, float a) {
		glTexEnvfModulateColor(r, g, b, a, true);
	}

	public final void glUpdateReplace(float r, float g, float b, float a) {
		glTexEnvfReplaceColor(r, g, b, a, true);
	}
