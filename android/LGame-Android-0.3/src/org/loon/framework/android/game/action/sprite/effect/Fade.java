...
public class Fade extends LObject implements ISprite {
	...
	public static Fade getInstance(int type, LColor c) {
		return new Fade(c, 60, type, LSystem.getSystemHandler().getWidth(),
				LSystem.getSystemHandler().getHeight());

	}

	public Fade(LColor c, int delay, int type, int w, int h) {
		this.visible = true;
		this.type = type;
		this.setDelay(delay);
		this.setColor(c);
