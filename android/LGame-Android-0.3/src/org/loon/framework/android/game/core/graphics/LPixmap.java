...
public class LPixmap extends LGraphicsMath {
	...
	public LPixmap(String fileName, boolean transparency) {
		Bitmap bit = GraphicsUtils.loadBitmap(fileName, transparency);
		this.width = bit.getWidth();
		this.height = bit.getHeight();
		LImage img = GraphicsUtils.filterBitmapTo565(bit, width, height);
		this.drawPixels = new int[width * height];
		img.getPixels(drawPixels, 0, width, 0, 0, width, height);
		if (img != null) {
			img.dispose();
			img = null;
		}
	...
	public LPixmap(int w, int h, LColor c) {
		this(w, h, c.getARGB());
	}
	...
	public LPixmap mirror(boolean mirror, boolean flip) {
		...
		LPixmap pixel = new LPixmap(width, height);
		if (pixel == null) {
			return null;
		}
