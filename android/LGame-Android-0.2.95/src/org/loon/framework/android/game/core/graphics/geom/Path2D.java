...
public abstract class Path2D implements Shape, Cloneable {
	...
	public static class Float extends Path2D implements Serializable {
		...
		public Float(Shape s, AffineTransform at) {
			if (s instanceof Path2D) {
				...
				this.pointTypes = CollectionUtils.copyOf(p2d.pointTypes,
		...
		float[] cloneCoordsFloat(AffineTransform at) {
			float ret[];
			if (at == null) {
				ret = CollectionUtils.copyOf(this.floatCoords,
		...
		void needRoom(boolean needMove, int newCoords) {
			...
			if (numTypes >= size) {
				...
				pointTypes = CollectionUtils.copyOf(pointTypes, size + grow);
			}
			...
			if (numCoords + newCoords > size) {
				...
				floatCoords = CollectionUtils.copyOf(floatCoords, size + grow);
			}
		}
	...
	public static class Double extends Path2D implements Serializable {
		...
		double[] cloneCoordsDouble(AffineTransform at) {
			double ret[];
			if (at == null) {
				ret = CollectionUtils.copyOf(this.doubleCoords,
		...
		void needRoom(boolean needMove, int newCoords) {
			...
			if (numCoords + newCoords > size) {
				...
				doubleCoords = CollectionUtils
						.copyOf(doubleCoords, size + grow);
			}
		}
