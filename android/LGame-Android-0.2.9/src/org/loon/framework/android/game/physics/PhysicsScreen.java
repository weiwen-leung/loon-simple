...
public abstract class PhysicsScreen extends Screen implements ContactListener,
	...
	private long remainderMicros, physicsFPS;
	...
	public PhysicsScreen(Vector2 gravity, boolean doSleep,
		...
		if (LSystem.isEmulator()) {
			PhysicsScreen.MAX_FPS = LSystem.DEFAULT_MAX_FPS / 2 - 10;
		} else {
			PhysicsScreen.MAX_FPS = LSystem.DEFAULT_MAX_FPS / 2;
		}
		this.physicsFPS = LSystem.DEFAULT_MAX_FPS;
	...
	public void run() {
		...
		do {
			...
			synchronized (objects) {
				...
				worldSleepTimeMicros = 1;
			}
			long currTimeMicros;
			if (physicsFPS == MAX_FPS) {
				if (worldSleepTimeMicros == 0) {
					try {
						Thread.sleep(1);
					} catch (InterruptedException e) {
					}
	...
	final public boolean onTouchDown(final MotionEvent e) {
	...
	final public boolean onTouchMove(final MotionEvent e) {
		onMove(e);
		return true;
	}

	public abstract void onMove(MotionEvent e);

	final public boolean onTouchUp(final MotionEvent e) {
		onUp(e);
		return true;
	}

	public abstract void onUp(MotionEvent e);
	...
	public void setPhysicsFPS(long physicsFPS) {
		...
		if (LSystem.isEmulator()) {
			PhysicsScreen.MAX_FPS = physicsFPS / 2 - 10;
		} else {
			PhysicsScreen.MAX_FPS = physicsFPS / 2;
		}
	}
