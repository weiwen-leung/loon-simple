...
public class FlashLabel extends LObject implements ISprite {
	...
	public FlashLabel(String s, int x, int y, LColor c) {
		this(s, x, y, LFont.getDefaultFont(), c);
	}

	public FlashLabel(String s, int x, int y) {
		this(s, x, y, LFont.getDefaultFont(), LColor.white);
	}

	public FlashLabel(String s, int x, int y, LFont font, LColor color) {
		this.visible = true;
		this.message = s;
	...
	public void createUI(LGraphics g) {
		...
		if (isFlashing) {
			...
			for (int k = 0; k < index; k++) {
				...
				g.setColor(new LColor(flashR, flashG, flashB, l));
				g.setFont(LFont.getFont(LSystem.FONT_NAME, i, j + dFontSize * (k + 1)));
