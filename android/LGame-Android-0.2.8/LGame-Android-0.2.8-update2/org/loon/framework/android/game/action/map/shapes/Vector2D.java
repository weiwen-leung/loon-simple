...
public class Vector2D implements Serializable {
	...
	public static Vector2D sum(List<Vector2D> summands) {
		Vector2D result = new Vector2D(0, 0);
		for (Iterator<Vector2D> it = summands.iterator(); it.hasNext();) {
			Vector2D v = it.next();
			result.addThis(v);
		}
		return result;
	...
	public static Vector2D mean(List<Vector2D> points) {
		int n = points.size();
		if (n == 0) {
	...
	public static Vector2D getRealCordinates(Vector2D relativeCoordinate,
			Vector2D centroid, double angularPosition) {
		Vector2D answer = (new Matrix2D(angularPosition)).multiply(
