...
public final class LGraphicsAndroid2D implements LGraphics {
	...
	private final static PaintFlagsDrawFilter drawFilter = new PaintFlagsDrawFilter(
	...
	private static float getDegree(float radian) {
	...
	public void rotate(double theta) {
		...
		matrix.reset();
		matrix.preRotate((float) getDegree((float) (RAD_360 - theta)));
	...
	public void rotate(double theta, double x, double y) {
		...
		matrix.reset();
		matrix
				.preRotate((float) getDegree((float) theta), (float) x,
						(float) y);
		canvas.concat(matrix);
	}
	...
	public void rotateCanvas(float degrees, float px, float py) {
		if (isClose) {
			return;
		}
	...
	public void clipPolygon(int[] xpoints, int[] ypoints, int npoints) {
		...
		try {
			...
			paint.setStyle(Style.FILL);
			Path p = new Path();
			p.moveTo(xpoints[0], ypoints[1]);
			for (int i = 2; i < npoints * 2; i += 2) {
				p.lineTo(xpoints[i], ypoints[i + 1]);
			}
			p.close();
