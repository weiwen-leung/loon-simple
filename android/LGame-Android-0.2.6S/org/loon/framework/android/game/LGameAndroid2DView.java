...
public class LGameAndroid2DView extends SurfaceView implements
	...
	public void update(LImage img, int w, int h) {
		...
		if (canvas != null) {
			synchronized (surfaceHolder) {
				canvas.drawBitmap(img.getBitmap(), width / 2 - w / 2, height
						/ 2 - h / 2, null);
			}
			surfaceHolder.unlockCanvasAndPost(canvas);
		}
	...
	public void updateFull(LImage img, int w, int h) {
		...
		if (canvas != null) {
			...
			if (nw == w && nh == h) {
				synchronized (surfaceHolder) {
					canvas.drawBitmap(img.getBitmap(), width / 2 - w / 2,
							height / 2 - h / 2, null);
				}
				surfaceHolder.unlockCanvasAndPost(canvas);
				return;
			...
			synchronized (surfaceHolder) {
				canvas.drawBitmap(img.getBitmap(), tmp_matrix, null);
			}
			surfaceHolder.unlockCanvasAndPost(canvas);
		}
	}
	...
	final private class CanvasThread extends Thread {
		...
		private void showLogo() {
			try {
				...
				try {
					if (logo == null) {
						logo = GraphicsUtils.loadNotCacheImage("system/images/logo.png",true);
					}
