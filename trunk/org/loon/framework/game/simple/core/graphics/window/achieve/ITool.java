...
import org.loon.framework.game.simple.core.graphics.window.LTool;
...
public class ITool extends IButton{
	...
	public void createUI(Graphics2D g, int x, int y, LComponent component, BufferedImage[] buttonImage) {
		LTool button = (LTool) component;
		if (!button.isEnabled()) {
			g.drawImage(buttonImage[3], x, y, null);
		}
		else if (button.isMousePressed()) {
			g.drawImage(buttonImage[2], x, y, null);
		}
		else if (button.isMouseOver()) {
			g.drawImage(buttonImage[1], x, y, null);
		}
		else {
			g.drawImage(buttonImage[0], x, y, null);
		}
	}
