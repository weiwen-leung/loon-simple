...
public class IPrint {
	...
	public IPrint(String context, Vector2D vector, int width, int height) {
		this.messageBuffer = new StringBuffer(messageLength);
	...
	private void drawMessage(LGraphics g, Color old) {
		...
		synchronized (showMessages) {
			...
			this.fontSize = isEnglish ? graphicsFont.getSize() / 2
					: graphicsFont.getSize();
			this.fontHeight = g.getLFont().getHeight();
			...
			for (int i = 0; i < size; i++) {
				...
				left += font;
				if (font <= 10 && StringUtils.isSingle(showMessages[i])) {
					left += 12;
				}
				if (i != size - 1) {
