...
public abstract class LComponent extends LObject implements LRelease {
	...
	public LComponent(int x, int y, int width, int height) {
		...
		this.width = width;
		this.height = height;
		this.screenRect = GameManager.getSystemHandler().getScreenBox();
		if (this.width == 0) {
			this.width = 10;
		}
		if (this.height == 0) {
	...
	protected void processMousePressed() {
		GameManager.getSystemHandler().getTextOrigin().setLocation(
		...
		if (this.customRendering) {
			...
			if (!this.processUI) {
				...
				for (int i = 0; i < this.ui.length; i++) {
					this.ui[i] = GraphicsUtils.createImage(this.imageUI[i]
							.getWidth(), this.imageUI[i].getHeight(),
							this.imageUI[i].getColorModel().getTransparency());
	...
	public void clearBackground() {
		this.setBackground(GraphicsUtils.createImage(1, 1,
	...
	public void setBackground(Color color) {
		BufferedImage image = GraphicsUtils.createImage(getWidth(),
