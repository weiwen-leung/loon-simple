...
import org.loon.framework.javase.game.core.graphics.window.actor.Actor;
...
public class MoveTo extends ActionEvent {
	...
	private List tmp_path;

	private int startX, startY, endX, endY, moveX, moveY;

	private int direction, speed;

	private final static HashMap pathCache = new HashMap(
	...
	public void onLoad() {
		...
		if (!original.getRectBox().contains(endLocation.x(), endLocation.y())) {
			synchronized (pathCache) {
				...
				List final_path = (List) pathCache.get(key);
				if (final_path == null) {
