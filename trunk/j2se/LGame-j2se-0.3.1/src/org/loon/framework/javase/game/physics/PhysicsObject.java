...
public class PhysicsObject implements PolygonType, ISprite {
	...
	private Triangle2f[] polyTriangles;
	...
	private int screenWidth = LSystem.screenRect.getWidth();

	private int screenHeight = LSystem.screenRect.getHeight();

	private final static HashMap<Integer, Triangle2f[]> lazyTriangles = new HashMap<Integer, Triangle2f[]>();

	PhysicsObject(String fileName, BodyType type, int x, int y) {
		this(GraphicsUtils.loadBufferedImage(fileName), 3, type, x, y);
	}

	PhysicsObject(String fileName, int polyInterval, BodyType type, int x, int y) {
		this(GraphicsUtils.loadBufferedImage(fileName), polyInterval, type, x,
				y);
	}
	...
	PhysicsObject(BufferedImage img, int polyInterval, BodyType type, int x,
			int y) {
		init(0, polyInterval, img, type, x, y, img.getWidth(), img.getHeight());
	}

	PhysicsObject(BufferedImage img, BodyType type, int x, int y) {
	...
	private void init(int polyType, int polyInterval, BufferedImage img,
		...
		if (img != null) {
			this.texture = new LTexture(GLLoader.getTextureData(img),
					Format.LINEAR);
		}
	}
			...
			try {
				...
				if (useImage) {
					...
					if (texture == null && image != null) {
						this.setDrawImage(image.getBufferedImage());
					}
					if (g != null) {
						g.dispose();
					}
	...
	public void setDrawImage(String fileName) {
		setDrawImage(GraphicsUtils.loadBufferedImage(fileName));
	}
