...
import org.loon.framework.javase.game.action.sprite.CollisionMask;
 ...
 * @version 0.1
 */
public class SpriteImage extends LObject {
	...
	public SpriteImage(BufferedImage bit) {
		this(bit, 0, 0);
	}

	public SpriteImage(LImage img) {
		this(img.getBufferedImage(), 0, 0);
	}

	public SpriteImage(BufferedImage bit, int x, int y) {
		this(bit, x, y, bit.getWidth(), bit.getHeight());
	}

	public SpriteImage(LImage img, int x, int y) {
		this(img.getBufferedImage(), x, y);
	}

	public SpriteImage(LImage img, int x, int y, int width, int height) {
		this(img.getBufferedImage(), x, y, width, height);
	}

	public SpriteImage(BufferedImage bit, int x, int y, int width, int height) {
	...
	private void bind(BufferedImage img) {
		bind(img, null);
	}

	private void bind(BufferedImage bit, Color color) {
		bitmap = bit;
		...
		for (int i = 0; i < size; i++) {
			pixels[i] = ColorUtils.premultiply(pixel = pixels[i]);
