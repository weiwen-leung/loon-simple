...
public class Fade extends LObject implements ISprite {
	...
	public static Fade getInstance(int type, GLColor c) {
		return new Fade(c, 120, type, LSystem.screenRect.getWidth(),
				LSystem.screenRect.getHeight());

	}

	public Fade(GLColor c, int delay, int type, int w, int h) {
		this.visible = true;
		this.type = type;
		this.setDelay(delay);
		this.setColor(c);
