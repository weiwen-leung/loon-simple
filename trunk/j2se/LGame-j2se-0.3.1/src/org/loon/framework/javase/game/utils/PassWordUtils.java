...
final public class PassWordUtils {
	...
	final static private HashMap<String, String> pswMapEmc = new HashMap<String, String>();

	final static private HashMap<String, String> pswMapDec = new HashMap<String, String>();
	...
	public static String toMD5(String origin) {
		...
		try {
			...
			result = PassWordUtils
					.toHexString(digest.digest(result.getBytes()));
		} catch (Exception ex) {
			return null;
		}
		return result;
	...
	public static byte[] longToByteArray(long value, byte[] data, int offset) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (i == 0 && value < 0)
				...
				data[offset + i] = (byte) (~data[offset + i] + (data[offset + i
						- 1] == 0 ? 1 : 0));
		}
	...
	public static int byteArraySearch(byte[] data, byte[] searchData) {
		for (int i = 0; i <= data.length - searchData.length; i++) {
			if (data[i] == searchData[0]
					&& Arrays.equals(searchData, extractBytes(data, i,
							searchData.length)))
				return i;
		}
	...
	public static float quadTan(double x, double y, boolean returnAsDegrees,
		...
		return (returnAsDegrees ? (float) (basic * 180.0 / Math.PI)
				: (float) basic);
	}
