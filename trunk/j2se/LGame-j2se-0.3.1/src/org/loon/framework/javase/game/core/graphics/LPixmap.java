...
import org.loon.framework.javase.game.core.geom.Polygon2i;
...
public final class LPixmap extends LGraphicsMath implements LRelease {
	...
	public LPixmap(String fileName) {
		try {
			pixelToImage = ImageIO.read(Resources.openResource(fileName));
	...
	public void setColor(int red, int green, int blue, int alpha) {
		...
		this.colorValue = ColorUtils.getARGB(red, green, blue, alpha);
	}
	...
	public int getARGB(int x, int y) {
		if (x < 0 || y < 0 || x >= width || y >= height) {
			...
			return ColorUtils.unpremultiply(drawPixels[x + y * width]);
		}
	}
