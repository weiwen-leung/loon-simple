...
public class LImage {
	...
	public LImage getCacheSubImage(int x, int y, int w, int h) {
		...
		if (img == null) {
			subs.put(key, img = new LImage(GraphicsUtils.drawClipImage(
					bufferedImage, w, h, x, y)));
		}
		return img;
	}

	public LImage getSubImage(int x, int y, int w, int h) {
		return new LImage(GraphicsUtils
				.drawClipImage(bufferedImage, w, h, x, y));
	}
	...
	public LImage scaledInstance(int w, int h) {
		...
		return new LImage(bufferedImage.getScaledInstance(width, height,
