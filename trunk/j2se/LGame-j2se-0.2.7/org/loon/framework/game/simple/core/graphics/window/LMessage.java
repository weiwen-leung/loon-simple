...
import org.loon.framework.game.simple.core.graphics.window.achieve.IPrint;
...
public class LMessage extends LContainer {
	...
	protected void processMouseDragged() {
		if (!locked) {
			...
			this.dx += this.input.getMouseDX();
			this.dy += this.input.getMouseDY();
	...
	public void setPauseIconAnimation(Animation animation) {
		...
		if (animation != null) {
			...
			if (image != null) {
				...
				this.setPauseIconAnimationLocation(getScreenX() + getWidth() + dw
						/ 2, getScreenY() + getHeight() - dh);
			}
		}
	}
