...
public abstract class ThreadScreen extends Screen implements Runnable {
	...
	private LinkedList[] firstLayer;

	private LinkedList[] firstLayerPts;

	private LinkedList[] firstLayerOpacity;
	...
	public ThreadScreen() {
		init(GameManager.getSystemHandler().getWidth(), GameManager
	...
	public ThreadScreen(int width, int height, int mapWidth, int mapHeight) {
	...
	private void init(int width, int height) {
		...
		this.awtImage = screen.getBufferedImage();
		this.drawGraphics = screen.getLGraphics();
		this.ascent = (int) drawGraphics.getFontMetrics().getAscent();
	...
	public void inicializaLayer() {
		for (int c = 0; c < this.firstLayer.length; ++c) {
			this.firstLayerPts[c] = new LinkedList();
			this.firstLayer[c] = new LinkedList();
			this.firstLayerOpacity[c] = new LinkedList();
		}
	...
	public void addLayer(LImage img, int layer, int x, int y, float opacity) {
		if (this.focus.intersects(x, y, img.getWidth(), img.getHeight())) {
			...
			this.firstLayerOpacity[layer].add(Float.valueOf(opacity));
		}
	}
	...
	public void putLayer(int layer) {
		while (!this.firstLayer[layer].isEmpty()) {
			this
					.putImage((LImage) this.firstLayer[layer].get(0),
							((Point) this.firstLayerPts[layer].get(0)).x,
							((Point) this.firstLayerPts[layer].get(0)).y,
							((Float) this.firstLayerOpacity[layer].get(0))
									.floatValue());
	...
	public void putFillRect(int x, int y, int width, int height, Color c1,
			Color c2) {
		if (this.focus.intersects(x, y, width, height)) {
			this.drawGraphics.setPaint(new GradientPaint((float) x, (float) y,
					c1, (float) (x + width), (float) y, c2, true));
			this.drawGraphics.fillRect(x - this.focus.getX(), y
					- this.focus.getY(), width, height);
		}
	...
	public void color(int r, int g, int b) {
		drawGraphics.setColor(new Color(r, g, b));
	}
	...
	public synchronized void repaint() {
		...
		if (match) {
			LSystem.repaintLocation(awtImage, centerX, centerY);
		} else {
			LSystem.repaintFull(awtImage, extWidth, extHeight);
		}
	}
