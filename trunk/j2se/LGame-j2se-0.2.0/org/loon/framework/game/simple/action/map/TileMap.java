...
public class TileMap {
	...
	public Image getTileFromPixels(double x, double y) {
		return getTileFromPixels(new Vector2D(x, y));
	}

	public Image getTileFromPixels(Vector2D p) {
	...
	public Vector2D getCollision(Vector2D p, double width, double height) {
		Image tile1 = getTileFromPixels(p.getX(), p.getY());
		Image tile2 = getTileFromPixels(p.getX(), p.getY() + height);
		Image tile3 = getTileFromPixels(p.getX() + width, p.getY());
		double x, y;
		x = y = 0;
		if (tile1 != null) {
			x = -1;
	 ...
	 * @param velocity
	 */
	public void setScrollingVelocity(Vector2D velocity) {
