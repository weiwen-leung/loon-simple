...
public abstract class LContainer extends LComponent {
	...
	private Comparator comparator = LContainer.DEFAULT_COMPARATOR;
	...
	public synchronized void add(LComponent comp) {
		...
		this.childs = (LComponent[]) LSystem.expand(this.childs, 1, false);
		this.childs[0] = comp;
		this.childCount++;
	...
	public synchronized LComponent remove(int index) {
		...
		this.childs = (LComponent[]) LSystem.cut(this.childs, index);
		this.childCount--;
		...
		for (int i = 0; i < this.childCount; i++) {
			...
			if (component != null) {
				if (component.isEnabled() && component.isFocusable()
					...
					if (component instanceof LText) {
						((LText) component).changeText(text);
					}
				}
			}
		}
	...
	public void sendToFront(LComponent comp) {
		...
		for (int i = 0; i < this.childCount; i++) {
			if (this.childs[i] == comp) {
				this.childs = (LComponent[]) LSystem.cut(this.childs, i);
				this.childs = (LComponent[]) LSystem.expand(this.childs, 1,
						false);
				this.childs[0] = comp;
				this.sortComponents();
				break;
