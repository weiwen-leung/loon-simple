...
public class LProgress extends LContainer {
	...
	protected void createCustomUI(LGraphics g, int x, int y, int w, int h) {
		...
		if (this.showMessage) {
			g.drawStyleString(mes, x + (w - font.stringWidth(mes)) / 2 + 2,
					(int) (y + ((h + font.getAscent()) / 2) - 2), fontColor,
					LColor.black);
		}
		g.setFont(oldFont);
		g.setColor(oldColor);
	...
	public void progressBackgound(LColor c) {
		...
		this.progressBackground = createBackground(c, getWidth(),
				getHeight());
	}
	...
	public void progressColor(LColor c1, LColor c2, boolean flag) {
		...
		this.progressImage = createProgressImage(c1, c2, flag, getWidth(), getHeight());
	}

	private LImage createBackground(LColor c, int w, int h) {
	...
	private LImage createProgressImage(LColor c1, LColor c2, boolean flag, int w,
			int h) {
