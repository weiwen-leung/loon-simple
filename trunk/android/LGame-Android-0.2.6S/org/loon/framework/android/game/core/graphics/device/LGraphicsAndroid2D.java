...
public final class LGraphicsAndroid2D implements LGraphics {
	...
	public void drawRegion(LImage src, int x_src, int y_src, int width,
		...
		case TRANS_ROT90: {
			matrix.preRotate(90);
			img = Bitmap.createBitmap(img, x_src, y_src, width, height, matrix,
					true);
			dW = height;
			dH = width;
			break;
	...
	public void drawMirrorImage(LImage img, int x, int y, boolean filter) {
		...
		Bitmap dst = mirrorImage.get(img.hashCode());
		if (dst == null) {
			tmp_matrix.reset();
			tmp_matrix.setValues(mirror);
			int width = img.getWidth();
			int height = img.getHeight();
			dst = Bitmap.createBitmap(img.getBitmap(), 0, 0, width, height,
					tmp_matrix, filter);
			mirrorImage.put(img.hashCode(), dst);
		}
		canvas.drawBitmap(dst, x, y, paint);
	}
	...
	public void drawString(String message, float x, float y) {
		...
		Path pth = new Path();
		paint.getTextPath(message, 0, message.length(), x, y, pth);
