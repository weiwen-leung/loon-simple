...
public class AdUtils {
	...
	public static void createAdView(LGameAndroid2DActivity context,
			Location lad, int requestInterval) {
		createAdView(context, lad, dealutKeywords, requestInterval, null);
	}

	public static void createAdView(LGameAndroid2DActivity context,
			Location lad, int requestInterval,
			com.admob.android.ads.AdListener listener) {
		createAdView(context, lad, dealutKeywords, requestInterval, listener);
	}

	public static void createAdView(LGameAndroid2DActivity context,
			Location lad, String keywords, int requestInterval) {
		createAdView(context, lad, keywords, requestInterval, null);
	}

	public static void createAdView(final LGameAndroid2DActivity context,
			final Location lad, final String keywords,
			final int requestInterval,
			final com.admob.android.ads.AdListener listener) {
		if (!isAdJarExist) {
			return;
		}
		if (adObject != null) {
		...
		LSystem.thread(new Runnable() {
			public void run() {
				try {
					if (LSystem.isEmulator()) {
						com.admob.android.ads.AdManager
								.setTestDevices(new String[] { com.admob.android.ads.AdManager.TEST_EMULATOR });
					}
	...
	public static void setInterstitialPublisherId(final String publisherId) {
		if (isAdJarExist) {
			com.admob.android.ads.AdManager
					.setInterstitialPublisherId(publisherId);
		}
	}
