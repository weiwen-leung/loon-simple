...
public class SpriteTiled extends LObject implements ISprite {
	...
	public SpriteTiled(LImage image, int columns, int rows, int tileWidth,
			int tileHeight) {
	...
	public void setStaticTileSet(LImage image, int tileWidth, int tileHeight) {
		...
		LImage[] tileImages = new LImage[columns * rows];
		int index = 0;
		for (int row = 0; row < rows; row++) {
			for (int column = 0; column < columns; column++) {
				LImage tile = LImage.createImage(tileWidth, tileHeight, true);
				LGraphics g = tile.getLGraphics();
	...
	public void createUI(LGraphics g) {
		...
		if (this.bufferGrid == null) {
			...
			this.bufferImage = LImage.createImage(this.cellWidth
					* ((clipWidth / this.cellWidth) + 2), this.cellHeight
					* ((clipHeight / this.cellHeight) + 2), false);
		...
		for (int column = firstColumn; column < lastColumn; column++) {
			...
			for (int row = firstRow; row < lastRow; row++) {
				...
				if (cellIndex != 0) {
					...
					LImage tile = this.tiles[tileIndex];
	...
	public RectBox getCollisionBox() {
		if (rect == null) {
			rect = new RectBox(getLocation().x(), getLocation().y(), width,
					height);
		} else {
			rect.setBounds(getLocation().x(), getLocation().y(), width, height);
		}
		return rect;
	}
