 ...
 * @version 0.1
 */
public class Actor implements ImageFilterType {
	...
	public final static int FADE_SPEED = 60;
	...
	public void setScale(final float s) {
		...
		if (image != null) {
			this.scale = s > 10 ? 10 : s;
			if (oldScale != null && scale == 1) {
				setImage(oldScale);
				return;
			}
			if (oldScale == null) {
				oldScale = image;
				LImage newImage = image.scaledInstance(
						(int) (getWidth() * this.scale),
						(int) (getHeight() * this.scale));
				setImage(newImage);
			} else {
				LImage tmp = image;
				image = oldScale.scaledInstance(
						(int) (oldScale.getWidth() * this.scale),
						(int) (oldScale.getHeight() * this.scale));
				tmp.dispose();
				setImage(image);
			}
		}
	...
	private void setRotateSheet(final Bitmap img) {
		...
		if (img.getConfig() == Config.RGB_565) {
			return;
		}
	...
	public RectBox getRectBox() {
		...
		if (rect == null) {
			rect = new RectBox(x, y, tmp.width, tmp.height);
		} else {
			rect.setBounds(x, y, tmp.width, tmp.height);
		}
		return rect;
	}
	...
	private void calcBounds() {
		ActorLayer layer = this.getLayer();
		if (layer != null) {
			int width;
			int height;
			...
			if (this.image == null) {
				...
				this.boundingRect = SpriteRotateSheet
						.getNewBounds(x, y, this.image.getWidth(), this.image
								.getHeight(), rotation);
			}
		}
	}
	...
	public int toPixel(int x) {
		ActorLayer aLayer = this.getLayer();
		if (aLayer == null) {
			return 0;
		} else {
	...
	public List<?> getNeighbours(int distance, boolean diagonal,
		...
		this.failIfNotInLayer();
		return this.getLayer().getNeighbours(this, distance, diagonal, cls);
	}
