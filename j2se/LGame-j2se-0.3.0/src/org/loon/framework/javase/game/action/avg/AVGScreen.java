...
import org.loon.framework.javase.game.action.sprite.effect.Fade;
...
import org.loon.framework.javase.game.core.graphics.CanvasScreen;
...
public abstract class AVGScreen extends CanvasScreen implements Runnable {
	...
	public AVGScreen(final String initscript) {
		...
		this.delay = 30;
		this.scriptName = initscript;
		this.dialog = GraphicsUtils.createImage(getCurrentWidth() - 40,
				getCurrentHeight() / 2 - 20, true);
		Graphics g = dialog.getGraphics();
		GraphicsUtils.setAlpha(g, 0.5D);
		GraphicsUtils.rectFill(g, 0, 0, dialog.getWidth(null), dialog
				.getHeight(null), Color.black);
	...
	public void onLoaded() {
		avgThread = new Thread(this);
		avgThread.setPriority(Thread.MIN_PRIORITY);
		avgThread.start();
	}
	...
	public void click() {
		...
		if (!isSelectMessage && sleep <= 0) {
			...
			if (message.isVisible()) {
				isNext = message.intersects(getMouseX(), getMouseY());
			} else {
				isNext = true;
			}
		} else if (scrFlag && select.getResultIndex() != -1) {
			onSelect(selectMessage, select.getResultIndex());
			isNext = select.intersects(getMouseX(), getMouseY());
			if (isNext) {
				message.setVisible(false);
				select.setVisible(false);
				isSelectMessage = false;
	...
	public void setDialogImage(BufferedImage dialog) {
		this.dialog = dialog;
	}
	...
	public Image getDialog() {
		return dialog;
	}

	public void setDialog(Image dialog) {
		this.dialog = dialog;
	}
