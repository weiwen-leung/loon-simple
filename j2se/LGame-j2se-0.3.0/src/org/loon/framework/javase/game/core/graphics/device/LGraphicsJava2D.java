 ...
 * @version 0.1
 */
public class LGraphicsJava2D extends Graphics2D implements LGraphics {
	...
	public LGraphicsJava2D(BufferedImage awtImage) {
	...
	public LGraphicsJava2D(VolatileImage awtImage) {
		this.width = awtImage.getWidth();
		this.height = awtImage.getHeight();
		this.g2d = awtImage.createGraphics();
		this.g2d.setClip(0, 0, width, height);
	...
	public void drawPixmap(LPixmap img) {
		drawImage(img.pixelToImage, img.getX(), img.getY());
	}

	public void drawPixmap(LPixmap img, int x, int y) {
		drawImage(img.pixelToImage, x, y);
	}
