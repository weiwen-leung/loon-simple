...
import org.loon.framework.javase.game.action.avg.CG;
import org.loon.framework.javase.game.action.avg.Chara;
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_CG)) {
						...
						if (mesFlag.equalsIgnoreCase(CommandType.L_DEL)) {
							...
							if (chara != null) {
								int x = chara.getX();
								int y = chara.getY();
								chara = new Chara(lastFlag, 0, 0, width);
								chara.setMove(false);
								chara.setX(x);
								chara.setY(y);
