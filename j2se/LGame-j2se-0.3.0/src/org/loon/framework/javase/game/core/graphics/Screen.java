...
import org.loon.framework.javase.game.action.sprite.SpriteListener;
...
import org.loon.framework.javase.game.core.LEvent;
...
import org.loon.framework.javase.game.core.graphics.window.actor.Layer;
...
public abstract class Screen extends SoundBox implements MouseListener,
	...
	static {
		...
		mousePressed = mouseReleased = new int[20];
		mouseMap = new HashMap(20);
	}
	 ...
	 * 返回与指定类匹配的组件
	 */
	public ArrayList getComponents(Class clazz) {
		if (desktop != null) {
	 ...
	 * 
	 */
	public ArrayList getSprites(Class clazz) {
		if (sprites != null) {
	...
	public synchronized void removeComponent(Class clazz) {
		if (desktop != null) {
	...
	public synchronized void removeSprite(Class clazz) {
		if (sprites != null) {
			...
			if (isVideo && videoThread == null) {
				try {
					...
					videoThread = new Thread(new Runnable() {
						public void run() {
							while (isVideo) {
								flvVideo.addFrame(GraphicsUtils
										.getBufferImage(getScreenImage()));
							}
						}
					});
					videoThread.start();
