...
import org.loon.framework.javase.game.srpg.actor.SRPGAnimation;
...
public abstract class SRPGScreen extends Screen implements SRPGType, Runnable {
	...
	private LFont choiceFont = LFont.getFont("Monospaced", 0, 15);
	 ...
	 * @return
	 */
	protected SRPGActor makeActor(SRPGStatus status, SRPGAnimation mv,
			SRPGAnimation atk) {
	 ...
	 * 
	 */
	protected abstract void processInitialize();
	...
	protected void makeCursor(int w, int h) {
		this.cursor = SRPGAnimation.makeCursor(w, h);
		this.isCursor = true;
	}
	...
	public final void setMessage(String[] message, int index, int offsetLeft,
		...
		try {
			...
			if (index != -1) {
				SRPGActor actor = srpgActors.find(index);
				Image img = actor.getActorStatus().face;
				if (img != null) {
	...
	public final synchronized void mouseDown(MouseEvent e) {
	...
	public final synchronized void mouseUp(MouseEvent e) {
		if (!isEventLoop || isSrpgTouchLock) {
			return;
		}
		if (e.getButton() == MouseEvent.BUTTON3) {
			onCancel(e.getX(), e.getY());
		}
		onUp(e);
	}

	public final synchronized void mouseMove(MouseEvent e) {
	...
	private synchronized final void callSRPGBattleProcEvent(
		...
		case PROC_NORMAL:
			// 确定
			if (type == SRPGEvent.EVENT_SUBMIT && isClick()) {
				// 角色存在
				int index = srpgActors.checkActor(x, y);
				if (index != -1) {
					SRPGActor actor = srpgActors.find(index);
	...
	private synchronized final void callSRPGBattleEvent(SRPGEvent e) {
		if (e.type == SRPGEvent.EVENT_SUBMIT && isClick()) {
	...
	public void onLoaded() {
		...
		srpgThread.setPriority(Thread.MIN_PRIORITY);
		srpgThread.start();
	}
