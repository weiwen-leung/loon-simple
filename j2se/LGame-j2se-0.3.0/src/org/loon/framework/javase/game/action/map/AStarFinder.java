...
public class AStarFinder implements Runnable {
	...
	private LinkedList calc(Field2D field, Vector2D start, Vector2D goal,
			boolean flag) {
	...
	private LinkedList astar(Field2D field, boolean flag) {
		for (; pathes.size() > 0;) {
			...
			for (int i = 0; i < size; i++) {
				...
				LinkedList path = new LinkedList(spath.path);
				path.add(next);
	...
	private void insert(int score, LinkedList path) {
	...
	private class ScoredPath {
		...
		ScoredPath(int score, LinkedList path) {
			this.score = score;
			this.path = path;
		}
