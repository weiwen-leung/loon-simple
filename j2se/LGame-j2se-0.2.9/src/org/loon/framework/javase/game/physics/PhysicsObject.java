...
public class PhysicsObject implements PolygonType, ISprite {
	...
	private static HashMap<Integer, Triangle[]> lazyTriangles = new HashMap<Integer, Triangle[]>();

	private static final AffineTransform atform = new AffineTransform();

	private int screenWidth = GameManager.getSystemHandler().getWidth();

	private int screenHeight = GameManager.getSystemHandler().getHeight();
			...
			if (useImage) {
				...
				case Box:
					...
					if (spriteRotate == null) {
						image = GraphicsUtils.createImage(width, height, false);
				...
				case Circle:
					...
					if (spriteRotate == null) {
						image = GraphicsUtils.createImage(width, height, true);
						g = image.createGraphics();
						g.setColor(color);
