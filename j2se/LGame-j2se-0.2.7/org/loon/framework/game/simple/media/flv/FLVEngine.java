...
public class FLVEngine {
	...
	public synchronized void addFrame(BufferedImage image, long elapsedTime) {
		try {
			container.writeFrame(GraphicsUtils.getResize(image, size),
					timer += elapsedTime);
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * @param sleep
	 */
	public static void outputFLV(String fileName, BufferedImage[] images,
			int sleep) {
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				container.writeFrame(GraphicsUtils.matchBufferedImage(
						images[i], size), timer += sleep);
			}
			out.close();
	 ...
	 * @throws IOException
	 */
	public static FLVContainer getFLVContainer(OutputStream out, int w, int h)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public static FLVContainer getFLVContainer(OutputStream out, Dimension size)
			throws IOException {
