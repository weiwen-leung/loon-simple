...
final public class GraphicsUtils {
			...
			for (int x = 0; x < wlength; x++) {
				...
				if (isFiltrate) {
					for (int i = 0; i < pixels.length; i++) {
						ColorUtils color = ColorUtils.getLColor(pixels[i]);
	...
	public static BufferedImage getBufferImage(Image image) {
		...
		if (w > 48 || h > 48) {
			...
			for (int i = 0; i < size; i++) {
				pixel = ColorUtils.premultiply(pixels[i]);
	...
	final static public BufferedImage loadBufferedImage(final String resName) {
		try {
			return ImageIO.read(Resources.openResource(resName));
		} catch (IOException e) {
			return null;
		}
	}
	...
	final static public BufferedImage loadDoubleFilterImage(final Image img,
		...
		for (int i = 0; i < length; i++) {
			if (basePixels2[i] >= ColorUtils.getPixel(200, 200, 200)) {
