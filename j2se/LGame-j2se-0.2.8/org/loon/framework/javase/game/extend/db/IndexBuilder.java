...
import org.loon.framework.javase.game.extend.db.index.IndexList;
import org.loon.framework.javase.game.extend.db.type.TypeUtils;
...
public class IndexBuilder {
	...
	final private static Map cacheMap = Collections.synchronizedMap(new HashMap(1000));
	...
	private IndexList getIndex(String indexName) {
		...
		try {
			...
			index = data
					.getIndex(indexName, TypeUtils.STRING, serializerValue);
			if (index == null) {
				index = data.makeIndex(indexName, TypeUtils.STRING,
						serializerValue);
			}
		} catch (IOException e) {
	...
	private IndexBuilder(final String fileName, final String password,
			final int type) {
	...
	public Set getKeys(String indexName) {
		...
		openIndex();
		keys = getIndex(
				StringUtils.replaceIgnoreCase(indexName, MDB.TAG_SUFFIX, ""))
				.getKeys();
		closeIndex();
		return keys;
	}
	...
	public void instert(String indexName, Object value) {
		openIndex();
		getIndex(StringUtils.replaceIgnoreCase(indexName, MDB.TAG_SUFFIX, ""))
				.put(indexName, value);
		closeIndex();
	}
