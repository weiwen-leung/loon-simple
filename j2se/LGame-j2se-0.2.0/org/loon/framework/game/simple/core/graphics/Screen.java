...
import org.loon.framework.game.simple.core.graphics.window.awt.AWTInputDialog;
import org.loon.framework.game.simple.core.graphics.window.awt.AWTMessageDialog;
import org.loon.framework.game.simple.core.graphics.window.awt.AWTOpenDialog;
import org.loon.framework.game.simple.core.graphics.window.awt.AWTYesNoCancelDialog;
import org.loon.framework.game.simple.core.store.IStore;
import org.loon.framework.game.simple.core.timer.LTimerContext;
import org.loon.framework.game.simple.media.flv.FLVEngine;
import org.loon.framework.game.simple.media.sound.SoundBox;
import org.loon.framework.game.simple.utils.GraphicsUtils;
import org.loon.framework.game.simple.utils.log.Level;
import org.loon.framework.game.simple.utils.log.Log;
import org.loon.framework.game.simple.utils.log.LogFactory;
...
public abstract class Screen extends SoundBox implements IScreen, LInput {
	...
	private BufferedImage currentScreen, tmp_screen;
	...
	public void resize() {
		...
		this.setBackground(GraphicsUtils.createIntdexedImage(tmp_width,
				tmp_width));
		this.tmp_screen = GraphicsUtils.createIntdexedImage(tmp_width,
				tmp_width);
		this.flvVideo = new FLVEngine(tmp_width, tmp_width);
		this.sprites = new Sprites(tmp_width, tmp_width);
		this.desktop = new Desktop(baseInput, tmp_width, tmp_width);
	}
	...
	public boolean onClick(ISprite sprite) {
		return sprite.isVisible()
				&& sprite.getCollisionBox().contains(mouseX, mouseY, 0,
						-sprite.getWidth());
	}
	...
	public boolean onClick(LComponent component) {
		return component.isVisible()
				&& component.getCollisionBox().contains(mouseX, mouseY, 0,
						-component.getWidth());
	}
	 ...
	 * 返回Screen存储器
	 */
	public IStore getScreenStore() {
		return ScreenManager.getScreenStore();
	}
	...
	public synchronized void runTimer(LTimerContext timer) {
		...
		if (this.desktop.getContentPane().getComponentCount() > 0) {
	 ...
	 * 创建程序UI
	 */
	public synchronized void createUI(final Graphics2D g) {
		draw(g);
		if (sprites != null) {
			sprites.createUI(g);
		}
	...
	public BufferedImage getScreenImage() {
		Graphics2D g = tmp_screen.createGraphics();
		...
		synchronized (keyLock) {
			...
			try {
				...
				if (!keyDown[code & 0xFF]) {
					...
					this.pressedKey++;
					if (keyMap.size() > 0) {
						Action action = (Action) keyMap.get(String
								.valueOf(code));
						if (action != null) {
							action.press();
						}
